...
exports.run = async(client, message, args) => {
  ...
  if (!args[0]) return message.channel.send(`:no_entry: Reklam Filtresini Ayarlamak İçin \`g!reklam aç\` | Kapatmak İstiyorsanız \`g!reklam kapat\` Yazabilirsiniz`)
  if (args[0] !== 'aç' && args[0] !== 'kapat') return message.channel.send(`:no_entry: Reklam Filtresini Ayarlamak İçin \`g!reklam aç\` | Kapatmak İstiyorsanız \`g!reklam kapat\` Yazabilirsiniz`)
  ...
  if (args[0] == 'kapat') {
    ...
    message.channel.send(`Reklam Filtresi başarıyla kapandı.`)
  }
...
exports.conf = {
 enabled: true,
 guildOnly: false,
  aliases: ['reklam', 'reklam-filtresi', 'reklamfiltresi', 'reklam-filtre', 'reklamfiltre', 'link-engelle'],
 permLevel: 0
};
